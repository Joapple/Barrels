...
public class DisplayItem {
	...
	public static void removeDisplayItem(Block b) {
		for(Entity n : b.getChunk().getEntities()) {
			if(n instanceof Item) {
				if(b.getLocation().add(0.5, 1.2, 0.5).distanceSquared(n.getLocation()) < 1D && ((Item) n).getItemStack().hasItemMeta() && ((Item) n).getItemStack().getItemMeta().getDisplayName().startsWith("§6§lB4R3L - §eITEM")) n.remove();
			}
		}
	}
	...
	private static Item getEntity(Block b) {
		for(Entity n : b.getChunk().getEntities()) {
			if(n instanceof Item) {
				if(b.getLocation().add(0.5, 1.2, 0.5).distanceSquared(n.getLocation()) < 1D && ((Item) n).getItemStack().hasItemMeta() && ((Item) n).getItemStack().getItemMeta().getDisplayName().startsWith("§6§lB4R3L - §eITEM")) return (Item) n;
			}
		}
		return null;
	}
