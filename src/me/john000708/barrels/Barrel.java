...
public class Barrel extends SlimefunItem {
	...
	private final int[] border1 = {0, 1, 2, 9, 11, 18, 19, 20};
	private final int[] border2 = {3, 5, 12, 13, 14, 21, 23};
	private final int[] border3 = {6, 7, 8, 15, 17, 24, 25, 26};
	
	protected int capacity;
	protected boolean allowDisplayItem;
	...
	public Barrel(Category category, ItemStack item, String name, RecipeType recipeType, final ItemStack[] recipe, int capacity) {
		...
		BlockMenuPreset blockMenuPreset = new BlockMenuPreset(name, getInventoryTitle()) {
			...
			public void newInstance(final BlockMenu menu, final Block b) {
				...
				registerEvent(new ItemManipulationEvent() {
					...
					@Override //we can do this 1st it shouldn't be long anyway
					public ItemStack onEvent(int i, ItemStack previous, ItemStack next) {
						updateBarrel(b);
						return next;//XXX if something is wrong return previous
					}//you did changes some stuff in here, so we might have to test it out tho
				...
				if(Barrels.displayItem) {
					Barrel.this.allowDisplayItem = b.getRelative(BlockFace.UP).getType() == Material.AIR;
					
					DisplayItem.updateDisplayItem(b, getCapacity(b), Barrel.this.allowDisplayItem);
				}
			}
		...
		registerBlockHandler(name, new SlimefunBlockHandler() {
			...
			public boolean onBreak(Player player, Block block, SlimefunItem slimefunItem, UnregisterReason unregisterReason) {
				...
				int storedAmount = Integer.valueOf(BlockStorage.getBlockInfo(block, "storedItems")).intValue();
				...
				if(inv.getItemInSlot(getInputSlots()[0]) != null) block.getWorld().dropItem(block.getLocation(), inv.getItemInSlot(getInputSlots()[0]));
				
				if(inv.getItemInSlot(getOutputSlots()[0]) != null) block.getWorld().dropItem(block.getLocation(), inv.getItemInSlot(getOutputSlots()[0]));
		...
		addItemHandler(new BlockTicker() {
			...
			public void tick(Block block, SlimefunItem slimefunItem, Config config) {
				...
				if(Barrels.displayItem) {
					...
					DisplayItem.updateDisplayItem(block, getCapacity(block), Barrel.this.allowDisplayItem);
				}
			}
		});
	...
	public int getCapacity(Block b) {
		...
		return Integer.valueOf(BlockStorage.getBlockInfo(b, "capacity")).intValue();
	}
	...
	private ItemStack getCapacityItem(Block b) {
		...
		int storedItems = Integer.valueOf(BlockStorage.getBlockInfo(b, "storedItems")).intValue();
	...
	protected void updateBarrel(Block b) {
		...
		for(int slot : getInputSlots()) {
			if(inventory.getItemInSlot(slot) != null) {
				...
				if(isSimiliar(input, inventory.getItemInSlot(22))) {
					...
					int storedAmount = Integer.valueOf(BlockStorage.getBlockInfo(b, "storedItems")).intValue();
		...
		int stored = Integer.valueOf(BlockStorage.getBlockInfo(b, "storedItems")).intValue();
	...
	protected static boolean isSimiliar(ItemStack i1, ItemStack i2) {
	...
	private static Inventory inject(Block b) {
	...
	protected static boolean fits(Block b, ItemStack[] items) {
		return inject(b).addItem(items).isEmpty();
	}
	
	protected static void pushItems(Block b, ItemStack[] items) {
